...
static void Motor_Dji_Decode(CanInstance_s *can_instance){
    ...
    if(motor->message.total_angle < 0)
        motor->message.total_angle += DJI_ECD_ANGLE_MAX*motor->reduction_ratio;
    else if(motor->message.total_angle > DJI_ECD_ANGLE_MAX*motor->reduction_ratio)
        motor->message.total_angle -= DJI_ECD_ANGLE_MAX*motor->reduction_ratio;
    motor->message.out_position = motor->message.total_angle*DJI_ECD_ANGLE_COEF/motor->reduction_ratio-PI;
}
